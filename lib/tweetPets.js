...
class TweetPets {
  ...
  getRandomPet() {
    return this.adoptPet.getRandomPet().then(randomPet => {
  ...
  generateText(pet) {
    ...
    const tweetText = `Hi! I'm ${name}, a ${sex} ${breed} ${species}. ${hashString} ${url}`;
  ...
  tweetSpecificPet(petId) {
    console.log('Preparing to tweet pet', petId);
    return this.adoptPet
      .getPetDetails(petId, true)
      .then(pet => {
        const text = this.generateText(pet);
        return this.twitter.tweet(text, pet.imageUrl);
      })
      .then(data => {
        console.log(`Tweet about ${petId} sent successfully`);
        return data;
      });
  }
}
